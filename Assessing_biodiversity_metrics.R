...
########## Fig.1A ##########
lit.scan <- read_xlsx("/Users/mingxin/PKU/Projects/1. Biases of mixing metrics/Scan/Scan_25_April_2023.xlsx", sheet = "Fig1A")
lit.scan$ecosystem <- factor(lit.scan$ecosystem)
lit.scan$habitat_change_type <- factor(lit.scan$habitat_change_type)
lit.scan$mixed_number <- factor(lit.scan$Number_of_mixed_number)
...
Fig.1A <- ggplot(df.1A, aes(x=mixed_status, y=total, fill=factor(mixed_number))) + geom_bar(position = position_stack(reverse = TRUE), stat="identity", width=0.9) + facet_grid(habitat_change_type ~ ecosystem, scales = "free") +
                            scale_fill_manual(values = c("#0868ac", "#43a2ca","#7bccc4", "#a8ddb5", "#ccebc5", "#fdd49e", "#fdbb84", "#fc8d59", "#e34a33", "#b30000")) + 
...
########## Fig.1C ##########
lit.scan <- read_xlsx("/Users/mingxin/PKU/Projects/1. Biases of mixing metrics/Scan/Scan_25_April_2023.xlsx", sheet = 4)
...
SF.OGF.raw <- read_xlsx("../../2. BES native forest restoration/Biodiversity/Miao/data_check_0404.xlsx", sheet = 1)
...
SF.OGF.comm.AMA.subset <- subset(SF.OGF.comm, SF.OGF.comm$LRR.AMA < 1.201303 & SF.OGF.comm$LRR.AMA > -1.717353)
...
SF.OGF.comm.GMA.subset <- subset(SF.OGF.comm, SF.OGF.comm$LRR.GMA < 1.337082 & SF.OGF.comm$LRR.GMA > -1.756334)
...
SF.OGF.comm.Simp.subset <- subset(SF.OGF.comm, SF.OGF.comm$LRR.Simp > -0.1564728 & SF.OGF.comm$LRR.Simp < 0.1255432)
...
SF.OGF.comm.simu5.subset <- subset(SF.OGF.comm, SF.OGF.comm$LRR.simu5 < 0.8123356 & SF.OGF.comm$LRR.simu5 > -1.0323629)
...
SF.OGF.comm.simu6.subset <- subset(SF.OGF.comm, SF.OGF.comm$LRR.simu6 < 0.8123356 & SF.OGF.comm$LRR.simu6 > -0.9098182)
